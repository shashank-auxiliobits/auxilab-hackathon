...
AP_JWT_SECRET=${JWT}
# Email OTP delivery: 'console' logs the code (default); 'smtp' sends it.
...
EOF
    ok "Wrote .env (key pepper + JWT secret generated)"
fi
...
if [ "$SEED" = true ]; then
    ...
    SEED_EMAIL="${AP_SEED_EMAIL:-owner-$(date +%s)@example.com}"
